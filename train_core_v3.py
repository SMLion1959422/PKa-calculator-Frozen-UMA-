"""Same pipeline as train_calibrated.py (5-fold OOF, isotonic
calibration, IDENTICAL LightGBM hyperparameters), pointed at the new
1536-dim (global+site-local) features instead of the original 768-dim
(global-only) ones. Hyperparameters are kept identical on purpose - the
only thing changing here is the feature set, so the MAE difference you
see is attributable to that change specifically, not a hyperparameter
tweak.
"""
import numpy as np
import pandas as pd
...
print("loading cached UMA features (v3: global+site-local, 1536-dim)...")
...
y = core.pKa.values
print(f"  {len(y)} labeled molecules with valid v3 features")
...
print(f"change: {raw_mae - cal_mae:+.3f} pKa units")
print(f"\n(this OOF number is on your training-label mix, NOT the same "
      f"external Novartis/AvLiLuMoVe set RESULTS.md reports 1.16-1.17 "
      f"on - run eval_core_v3.py next for the like-for-like comparison)")
...
joblib.dump({"regressor": final_model, "calibrator": calibrator}, "models/model_core_v3.pkl")
print("\nsaved -> models/model_core_v3.pkl")
